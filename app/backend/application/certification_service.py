"""Certification entry business logic."""
...
class CertificationService:
    """Service handling CRUD for specialist certification entries."""
    ...
    async def list_mine(self, user_id: UUID) -> list[dict]:
        """Return all certification entries for the calling specialist."""
    ...
    async def add_mine(self, user_id: UUID, payload: dict) -> dict:
        """Add a new certification entry for the calling specialist."""
        profile_id = await self._resolve_profile_id(user_id)
        _validate(payload)
    ...
    async def update_mine(self, user_id: UUID, entry_id: UUID, payload: dict) -> dict:
        ...
        if not entry:
            raise AppException(status_code=404, message="Certification entry not found")
