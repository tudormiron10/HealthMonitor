"""Abstract repository for specialist work experience entries."""
...
class WorkExperienceRepository(ABC):
    """Port defining CRUD operations for specialist_work_experience rows."""

    @abstractmethod
    async def add(self, specialist_profile_id: UUID, data: dict) -> dict:
        """Insert a new work experience entry and return the persisted row."""

    @abstractmethod
    async def list_for_specialist(self, specialist_profile_id: UUID) -> list[dict]:
        """Return all entries for a specialist ordered by display_order ASC, start_date DESC."""

    @abstractmethod
    async def get_by_id(self, entry_id: UUID) -> dict | None:
        """Return a single work experience entry by its ID, or None if not found."""
