...
function planToViewData(plan: Plan): PlanViewData {
  return {
    ...
    sender_name: plan.sender_name || undefined,
...
export const MyPlansPage = () => {
  ...
  const fetchPlans = async () => {
    try {
      const data = await plansApi.getMyPlans(true);
      setPlans(data);
      const deepLinkId = searchParams.get('planId');
      if (deepLinkId) {
        const target = data.find((p) => p.message_id === deepLinkId);
        if (target) setSelectedPlan(planToViewData(target));
      }
    } catch {
      setError(t('myPlans.error'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-brand-light rounded-3xl p-10 md:p-12 shadow-inner border border-brand-dark/5">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-brand-dark/60 mt-1">{t('myPlans.pageSubtitle')}</p>
          </div>
        </div>
      </div>
      ...
      <section>
        ...
        {active.length === 0 ? (
          <div className="bg-white/60 rounded-2xl border border-brand-dark/10 p-10 text-center">
            ...
            <p className="text-brand-dark/40 font-heading text-lg">{t('myPlans.emptyActive')}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {active.map((plan) => (
              <PlanRow
                key={plan.message_id}
                plan={plan}
                actionLabel={t('myPlans.archiveButton')}
            ...
            <div className="space-y-3">
              {archived.map((plan) => (
                <PlanRow
                  key={plan.message_id}
                  plan={plan}
                  actionLabel={t('myPlans.restoreButton')}
...
function PlanRow({ plan, actionLabel, actionIcon, isActioning, onAction, onClick, muted }: PlanRowProps) {
  const { t } = useTranslation();
